...
import type { CommentNode } from "@/app/blog/[slug]/api/get-comments";
...
export function CommentSection({ blogDocumentId, initialComments, loadError }: Props) {
  ...
  async function postComment(content: string, threadOf?: number): Promise<boolean> {
    ...
    const res = await fetch(`/api/blog-comments/${blogDocumentId}`, {
      method: "POST",
  ...
  async function reportComment(commentId: number, reason: string, content: string): Promise<boolean> {
    const res = await fetch(`/api/blog-comments/${blogDocumentId}/${commentId}/report`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <section className="mt-8 w-full">
      <h2 className="text-xl font-semibold mb-4">Comments ({countComments(initialComments)})</h2>
