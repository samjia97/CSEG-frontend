...
export async function getComments(documentId: string, token: string): Promise<CommentsResult> {
  try {
    const relation = `api::forum-thread.forum-thread:${documentId}`;
    ...
    if (!res.ok) {
      ...
      return { comments: [], error: `Couldn't load replies (${reason}).` };
    }
    ...
    return { comments: [], error: "Couldn't load replies (network error)." };
  }
}
